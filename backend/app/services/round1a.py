...
# Inline port of Round 1A analyze functions (imported from user's provided code)
import fitz
...
def analyze_pdf(pdf_path):
    try:
        ...
        return {"title": title, "outline": outline, "language": doc_language}
    except Exception as e:
        return {"title": "", "outline": [], "error": str(e)}


def analyze_pdf_to_outline(pdf_path: str) -> Dict[str, Any]:
    data = analyze_pdf(pdf_path)
    # persist sidecar outline for listing
    doc_id = os.path.basename(pdf_path).split("_", 1)[0]
    os.makedirs(META_DIR, exist_ok=True)
    sidecar = os.path.join(META_DIR, f"{doc_id}.outline.json")
    try:
        with open(sidecar, "wb") as f:
            f.write(orjson.dumps(data.get("outline", [])))
    except Exception:
        pass
    return data